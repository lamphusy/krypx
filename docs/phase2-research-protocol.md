...
synthetic-only source/test implementation and governance, not real feature use or a push.
...
### Historical Milestone 4 specification-only freeze
...
`config/phase2_protocol.json`. Only those three files could change in that step. The exact
13 feature formulas, types, bounds, window boundaries, relevance floor, share cutoffs and
half-lives below remain frozen. The September 15 freeze itself was not implementation
permission or approval of research outcomes.
...
At the freeze, `milestone_4_status` was `SPECIFICATION_FROZEN`, while both
`milestone_4_implementation_authorized` and `milestone_4_feature_aggregation_authorized`
were `false`. No code or tests were created by that freeze. The unchanged specification
document and JSON `milestone_4_specification` object retain that historical authority.
...
raw-byte replay, requires synthetic-only rights and synthetic prospective terminal
intervals, and rejects real/historical provider inputs. Caller-supplied article lists,
group anchors or coverage reports are not trusted inputs. The verified contiguous ledger
must cover the entire 24-hour trailing window **including the minute starting at t**;
missing coverage raises `AggregationCoverageError`, never a fabricated zero-news row.
Every terminal event and operational due time must be valid at the pinned coverage `as_of`.

Only fully replay-verified Milestone 3 `ScoreArtifact` buffers enter the frozen mock corpus.
They bind to normalized fixture articles by exact content hash, BTC, source and title;
the synthetic scoring `article_version_id` is not relabeled as a provider version ID.
Different scripted outcomes have different mock configuration hashes, all pinned in
lineage. Competing scores for one content hash fail closed. Unused verified mock artifacts
may remain in this frozen corpus when future-only articles are removed by perturbation;
they never contribute to a row without a matching eligible article version.

The grouped/bisected implementation preserves `(group_first_seen_at, duplicate_group_id)`
order and scalar binary64 arithmetic. It emits exactly the 13 frozen features, with
permanent anchor arrival/source, latest-version selection, no earlier-score fallback,
positive zero and typed float hydration. `scored_at` is audit-only. Diagnostics use all
selected eligible groups as the failure-rate denominator; missing or nonsucceeded scores
are failures, low relevance is reported separately. Verified gap rows retain their decision
and `provider_gap_window` reason but have **no feature payload**, not an invented neutral row.
Only a synthetic decision grid is used; no market data, model index or folds are created.

`AggregationStore` publishes in the same explicitly supplied CAS root as its immutable
provider parents. Exact row, configuration, score-buffer and lineage bytes are hashed,
with atomic manifest-last no-overwrite publication. Every public aggregation, publication
and reload re-verifies parent state/raw bytes and mock score evidence. Publishing/loading
recomputes the feature payload; altered rows or metadata cannot be legitimized merely by
recomputing hashes. Candidate `AggregationArtifact.rows` performs structural inspection,
not parent certification; verified artifacts must come from aggregation or store operations.
Complete corpus/state lineage belongs to the run envelope, never earlier feature-row bytes.
No build-time clock is minted on deterministic reruns.

Runtime and machine are pinned for ordered `math.pow`/`math.sqrt` binary64 operations;
cross-platform bit identity is not claimed without differential verification. The test
reference independently enumerates groups/versions and performs each formula explicitly.
Future-only article additions/removals/modifications, exact window/relevance/share edges,
missingness, gap evidence, schema/hash tampering and immutable publication behavior are
covered with synthetic fixtures and temporary stores. No repository pilot/scored data is
used or populated. Live deferral, no backfill, disabled network, and all real-scoring,
market-data, training/backtest and holdout prohibitions remain in force.

#### Milestone 4 offline engineering verification (2026-09-16)

These results cover the uncommitted implementation against `683f76d97e132c0d245391c82317b163d61f2e32`.
They are not independent acceptance or authority to use real features. All runs use
synthetic fixtures and temporary storage; existing synthetic Phase 1 pipeline/backtest
unit tests are regression verification, not execution of a research backtest.

| Check | Observed result |
|---|---|
| `git diff --check`, plus untracked-file whitespace checks | Passed |
| `.venv/bin/black --check .` | 84 files unchanged |
| `.venv/bin/ruff check --no-cache .` | All checks passed |
| `.venv/bin/python -m compileall -q src tests` | Exit 0; no output |
| `.venv/bin/python -m pip check` | No broken requirements; existing unwritable-cache warning only |
| `.venv/bin/pytest tests/sentiment/test_aggregation.py -x -q` | 127 passed in 57.41s |
| `.venv/bin/pytest --ignore=tests/sentiment/test_aggregation.py` | All 1,003 pre-existing tests passed; 12 existing warnings; 45.25s |
| `.venv/bin/pytest --ignore=tests/sentiment` | 253 passed; 12 existing warnings; 4.28s |
| `.venv/bin/pytest` | 1,130 passed; 12 existing warnings; 101.01s |
| Independent inline binary64 arithmetic oracle | 4,000/4,000 reductions bit-identical |
| Strict repository JSON / JSONL | 2/2 JSON documents and 2/2 JSONL records valid; 6/6 invalid controls rejected |
| RFC 8785 differential against local Node | 49,972/49,972 binary64 cases matched |
| Byte identity against baseline | 107/107 other existing tracked blobs unchanged, including all 82 existing source/test blobs and all 50 Phase 1 source/test blobs |

The only changed files are the new aggregator, its new test module, and the two authorized
governance files. The frozen Milestone 4 document/numerical contracts, historical pilot
...
1. Preserve the compressed file exactly. Immediately after its final byte arrives record `ingested_at`; after SHA-256 verification and successful atomic no-overwrite publication record `raw_published_at`. Also record filename timestamp, byte count, source URL, parser version, `gdelt-gsg-parser-policy-v1`, and its exact compressed-byte, decompressed-byte, and JSON-line bounds. Those bounds participate in `gdelt-gsg-snapshot-v2` identity so the same bytes cannot change parse outcome under the same snapshot ID. Distinct raw snapshot publications must receive canonical, strictly increasing `raw_published_at` values.
...
3. A single deterministic normalizer accepts only a contiguous retrieval plan beginning at its durable filename-interval watermark, `next_expected_interval_start`. Every minute must become either `retrieved_and_normalized` or a provider gap backed by verified immutable terminal evidence before that watermark advances. A separate exclusive causal boundary, `closed_availability_through`, is one UTC microsecond after the last terminal event: `raw_published_at` for a complete snapshot or the verified evidence's `terminal_at` for a missing/invalid gap. Terminal event times must strictly increase. Regressing, overlapping, replayed, pending, forward-gapped, availability-regressing, or equal-time distinct-snapshot plans fail before mutation.
...
- Normalizer state v3 is exported as canonical RFC 8785 files plus a canonical state index. The index transitively hashes the article versions, observation links, exclusions/conflicts, permanent group anchors, approval, protocol hash, `gdelt-gsg-terminal-chronology-v2` ledger, `closed_availability_through`, and canonical terminal gap evidence. The complete bundle is published atomically, manifest-last, and without replacement.
- The filename chronology from commit `8bd1050` remains intact. Its ledger stores one canonical minute record for every terminal interval through `next_expected_interval_start`, the exclusive filename watermark. Complete records bind snapshot ID, receipt timestamps, raw SHA-256, and line count. Missing and invalid records bind a canonical gap-evidence ID and SHA-256. Records are strictly ordered, contiguous, and immutable; every overlap and replay remains intentionally rejected.
- Causal chronology is a separate invariant. Each complete interval's terminal event is its canonical `raw_published_at`; each verified missing/invalid gap's terminal event is its evidence `terminal_at`. Terminal events, and separately the publication times of distinct delivered snapshots, must strictly increase. `closed_availability_through` equals the final terminal event plus one UTC microsecond, is persisted rather than inferred from article rows, and advances only with a fully validated terminal batch. A submitted snapshot publication before that exclusive boundary is rejected before article or group mutation, so no immutable duplicate-group anchor is reassigned or backdated.
- A missing or invalid snapshot is never inferred to be a provider gap from absence plus caller-supplied time. It requires immutable `gdelt-gsg-terminal-gap-evidence-v1` bound to provider, scope, exact interval, expected source locator, synthetic/prospective mode, input class, `gdelt-gsg-retry-policy-v1`, attempt count, and ordered attempt facts containing attempt number/time, HTTP status or bounded error kind, retry disposition, and Retry-After when applicable. It also binds final terminal disposition/time and the protocol hash. Invalid-snapshot evidence additionally binds snapshot ID, raw SHA-256, and bounded parser error. Only a verified non-retryable terminal outcome or exact retry exhaustion closes the interval. `evidence_id` hashes the RFC 8785 identity envelope; `evidence_sha256` hashes the canonical evidence body including that ID and excluding only the checksum field. The full canonical `gap-evidence.json` has its own exact-byte descriptor transitively covered by `state_sha256`; synthetic evidence grants no network authority.
- Hydration rejects every legacy state and any missing, malformed, noncanonical, duplicated, discontinuous, contradictory, or hash-mismatched state without migration, inference, or repair. It uses single-read manifest-verified buffers and verifies transitive descriptors, raw objects and receipts, parser policy, both watermarks, complete publication timestamps, gap evidence, and closed-world relationships among ledger records, articles, links, exclusions, receipts, and evidence. It reparses each exact raw buffer under its hashed bounds and chronologically replays normalization; a self-consistent state that disagrees with raw semantics is rejected. Successful hydration re-exports byte-identical canonical state.
...
- Equal-time behavior is frozen at raw-snapshot scope. If multiple fingerprints for one `article_id` share one availability timestamp inside one raw snapshot, every involved observation receives primary exclusion `revision_time_unknown`, and no conflicting version is created. Equal `raw_published_at` across distinct snapshots is invalid. If a conflict reaches already-published immutable state, normalization raises a project-specific integrity error, leaves that state unchanged, and publishes no replacement or rewritten history. This fail-closed generation must not be modeled.
- Canonical state bytes are partition- and restart-independent only for valid streams that satisfy both filename and causal-availability chronology. Invalid availability regression, equal-time distinct snapshots, or a conflict against immutable prior state fail closed and are not claimed to match an atomic rebuild. Recovery would require a separate chronological state generation outside this task.
...
Historical Milestone 0 proposed hard budget controls, all requiring human approval.
The one-GiB/seven-day raw-pilot proposal below is superseded for Batch B by the frozen
500 MB download, 2.0 GB storage, 900-second process, 24-hour/96-interval specification.
The unrelated scoring/query/research proposals remain unapproved:
...
requirements supplement this section. The separate September 16 amendment authorizes
synthetic-only implementation and tests, not real research use.
...
The aggregation parameters remain frozen for the offline Milestone 4 specification;
synthetic implementation is authorized, while real research use is not. The real scorer, collector
...
The prior offline verification for `21e954d24f6639da8c4d902924ebf01ffba715d8`
recorded 27 passing article-contract tests, 15 passing immutable-storage tests, 24 passing
GSG adapter/publication tests, 24 passing state-integrity tests, 179 passing Phase 2 sentiment
tests, 253 passing frozen Phase 1 tests, and 432 passing repository-wide tests with the same
12 expected Phase 1 single-class metric warnings. Diff, formatting, lint, compilation,
dependency, RFC 8785 differential, JSON, and Markdown/JSON reconciliation checks passed.
Those are historical results, not current acceptance: the next independent review returned
**NOT ACCEPTED** after reproducing caller-forged coverage and terminal gaps, incomplete
normalization provenance, state-v3 parsing before field-level metadata validation,
...
parsing state v3; inventories and rejects every unmanifested regular or non-regular object;
and uses a filesystem-root-anchored descriptor walk for configured-root creation followed by
descriptor-anchored fail-closed traversal from the verified trusted-root identity for both
reads and mutations. Coverage is recomputed only after exact receipt/CAS replay. Full
chronological raw-byte replay must reproduce the exact committed state and current-batch
result, including every reused link and exclusion. Every
published article carries exactly one non-reused representative link; prior-batch
representatives are raw-replayed and persisted separately from current-batch links. The
canonical normalized bundle therefore includes the retrieval plan, current snapshot
references, terminal gap evidence, representative links, and representative snapshot
references as `retrieval-plan.json`, `snapshot-references.json`,
`terminal-gap-evidence.json`, `representative-observation-links.jsonl`, and
`representative-snapshot-references.json`. Staging creation, nested writes, hard links,
cleanup, fsync, and atomic
...
`origin/codex/phase2-foundation` at `2026-08-16T16:59:32+07:00`; local `main` was
fast-forwarded at `2026-08-16T16:59:45+07:00`; and `origin/main` was updated at
`2026-08-16T16:59:50+07:00`. Commit
`21e954d24f6639da8c4d902924ebf01ffba715d8` later updated
`origin/codex/phase2-foundation` at `2026-08-27T22:56:48+07:00`; merge commit
`587e4475114db8b65fe47a7a8b2197952f2b9e75` was created through PR #2 at
`2026-08-27T22:58:18+07:00`; and `origin/main` and local `main` later contain that merge, with
...
### Batch B specification — historical v2 and offline implementation record

This section preserves the pre-live specification and implementation history. Its September 9
schedule, mock-only flags and then-pending approvals are historical, superseded operationally
by the separate September 13 live authority above. They do not restrict the new authorized
setup or permit synthetic evidence to masquerade as real responses. The offline corrective
commit `14769aff` subsequently passed independent acceptance with 723 tests; the initial
648-test review failure below remains part of the audit trail.
...
The action at that record was `independent_offline_batch_b_acceptance_review`. The September 10
...
The human selected Option 1, GDELT GSG archive, replacing the DOC API. The sole permitted
source for a later authorized pilot is
`https://data.gdeltproject.org/gdeltv3/gsg/{YYYYMMDDHHMMSS}.gsg.json.gz`, exactly as
generated by accepted Batch A `plan_retrieval()` and `expected_gsg_source_locator()`.
The instruction's HTTP `gdeltv2/` and `.gsg.jsonl.gz` spelling is corrected to the code's
HTTPS `gdeltv3/gsg/` and `.gsg.json.gz` locator to honor its explicit compatibility
decision; the gzip contents are JSONL. Neither that HTTP/v2 spelling nor the previous
`https://api.gdeltproject.org/api/v2/doc/doc` endpoint is allowlisted. Permit only the
...
900 seconds. First worker: `2026-09-09T00:45:00Z`; last worker:
`2026-09-10T00:30:00Z`; final retrieval and signed closeout deadline:
`2026-09-10T00:45:00Z`. This 45-minute tail does not admit next-day file timestamps or
extend the observation window. The lag is an accepted engineering policy, not a newly
verified provider SLA. A missed anchor cannot be backfilled, automatically shifted, or
evaluated against a smaller denominator. A late worker cannot extend its fixed deadline;
missed slots cannot be relaunched to create catch-up time.

| Pilot requirement | Frozen value |
|---|---|
| Aggregate download | 500 MB = 500,000,000 bytes, including partial/error/retried responses |
...
The document specifies exact raw-response CAS retention, Ed25519 collector-signed receipt
...
rights record remains required; the 2026-08-12 source register was not refreshed offline.

The DOC/GSG product blocker is resolved. Compatible accepted contracts are provider
`gdelt_gsg`, scope `gdelt_gsg_english_btc_titles`, parser `gdelt-gsg-jsonl-v1`, policy
`gdelt-gsg-parser-policy-v1`, snapshot `gdelt-gsg-snapshot-v2`, normalizer
`gdelt-gsg-normalizer-v3`, state `gdelt-gsg-normalizer-state-v3`, chronology
`gdelt-gsg-terminal-chronology-v2`, gap evidence `gdelt-gsg-terminal-gap-evidence-v1`,
and retry policy `gdelt-gsg-retry-policy-v1`. Observation IDs retain the exact filename
...
Batch A first-seen timestamps. Archive compatibility did not mean a live collector was
ready: offline implementation readiness at that record was `CORRECTED_OFFLINE_PENDING_INDEPENDENT_REVIEW`.
...
At that historical record, `network_pilot_authorized: false` and
`milestone_3_scoring_authorized: false` were in force; the separate September 13 authority
changes only the bounded network/rights permission, not scoring authorization.
...
appear in the verification record; fresh independent acceptance subsequently passed for
`14769aff`, without certifying live collection or extending the mock-only contracts.
...
The later local `origin/main` reflog records `14769aff` updated by push at
`2026-09-12T23:45:28+07:00`, without establishing actor or authorization. Current live setup
performs no push, fetch or live remote query. No live pilot completion or acceptance is
claimed before collection and independently verified signed closeout evidence exist.
